/**
 * JWT authentication middleware (async). Verifies the Bearer token and attaches
 * a lightweight `req.user` ({ id, role, fullName, email }) for downstream handlers.
 */
...
export async function authenticate(req, _res, next) {
  ...
  try {
    ...
    const user = await users.findByIdWithRole(payload.sub);
